<h1 align="center">AmanDeep Saini Singh</h1>

<p align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=Poppins&size=20&pause=1000&color=2F81F7&center=true&vCenter=true&width=600&lines=Data+Engineer+%7C+BI+Developer+%40+Data+Kingdom;MSc+Applied+Data+Science%2C+Utrecht+University;Building+data+platforms+on+Microsoft+Fabric;Machine+Learning+%26+NLP+for+Healthcare+Analytics" alt="Typing SVG" />
  </a>
</p>

<p align="center">
  <strong>Building data solutions across data engineering, business intelligence, and applied machine learning.</strong>
</p>

<p align="center">
  <a href="https://sainiamandeepsingh.github.io/"><img src="https://img.shields.io/badge/Portfolio-2F81F7?style=flat-square&logo=googlechrome&logoColor=white" alt="Portfolio"></a>
  <a href="https://www.linkedin.com/in/singhamandeepsainikahma/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white" alt="LinkedIn"></a>
  <a href="mailto:singhamandeepsaini@hotmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white" alt="Email"></a>
  <a href="https://github.com/SainiAmandeepSingh"><img src="https://komarev.com/ghpvc/?username=SainiAmandeepSingh&style=flat-square&color=2F81F7&label=Profile+views" alt="Profile views"></a>
</p>

<p align="center">
  <img src="https://skillicons.dev/icons?i=py,r,mysql,js,azure,d3,git,githubactions&theme=dark&perline=8" alt="Skill icons" />
</p>

---

### About

I am a Medior Data Engineer & BI Developer at [Data Kingdom](https://datakingdom.nl), building and maintaining data platforms on Microsoft Fabric alongside Power BI reporting solutions. I hold an MSc in Applied Data Science from Utrecht University (2025–2026), completed with a thesis at Amsterdam UMC.

I hold a Bachelor of Applied Science in ICT (Data Analyst and Scientist profile) from Fontys University of Applied Sciences and have over three years of professional experience across data engineering, business intelligence, and data science roles.

### Current work

**Data platform engineering at Data Kingdom.** Medior Data Engineer building and maintaining data platforms on Microsoft Fabric, supporting Jewa Group and CHDR with dashboarding and centralising their data landscape into scalable, Fabric-based reporting solutions.

**MSc thesis at Amsterdam UMC.** Predicted post-operative complications following cancer surgery using machine learning and natural language processing on pre-operative physiotherapy assessments and clinical notes, supervised by Dr. Mihaela Mitici (Utrecht University) and Dr. Edwin Geleijn (Amsterdam UMC).

**Coursework at Utrecht University.** Visual Analytics for Big Data, Personalisation for (Public) Media, Text and Media Analytics, Epidemiology and Medical Data Science, Data Wrangling, Data Analysis.

### Selected projects

<table>
<tr>
<td width="50%" valign="top">

#### [EvidenceBoard](https://github.com/SainiAmandeepSingh/EvidenceBoard)

A Marimo and D3.js visual analytics system for knowledge graph investigation, developed individually for the Utrecht University Design Challenge. Implements three coordinated views with computational ghost link detection, conflict entity identification, and an investigation queue.

</td>
<td width="50%" valign="top">

#### [PublicMedia](https://github.com/SainiAmandeepSingh/PublicMedia)

A Streamlit recommender system prototype for NPO Start, integrating fairness, diversity, transparency, and autonomy as public values. Implements an Exposure Gap fairness metric with broadcaster-aware re-ranking aligned with Mediawet 2008 requirements.

[Live demo →](https://nporecommender.streamlit.app/)

</td>
</tr>
<tr>
<td width="50%" valign="top">

#### [SignalTrace](https://github.com/SainiAmandeepSingh/SignalTrace)

Visual analytics dashboard developed for VAST Challenge 2025 Mini-Challenge 3. Combines Marimo and D3.js with `gpt-4o-mini` message classification and BERTopic topic modelling. Three-person team project.

</td>
<td width="50%" valign="top">

#### [Lumine](https://github.com/SainiAmandeepSingh/Lumine)

A Streamlit application combining TheMealDB API with local LLMs (gemma:2b and llama2 via Ollama) to generate recipe recommendations from user-uploaded grocery data. Four-person team project.

[Live demo →](https://budgetbites.streamlit.app/)

</td>
</tr>
<tr>
<td width="50%" valign="top">

#### [Spotify Recommender](https://github.com/SainiAmandeepSingh/Spotify)

A song recommendation system trained on the Million Song Dataset, with a Streamlit interface for live playlist generation. Developed during the Big Data and Design minor at HU University of Applied Sciences Utrecht.

[Live demo →](https://spotifyradio.streamlit.app/)

</td>
<td width="50%" valign="top">

#### [Personal portfolio](https://github.com/SainiAmandeepSingh/SainiAmandeepSingh.github.io)

Personal portfolio website built with vanilla HTML, CSS, and JavaScript. Mobile-first dark theme with full responsive support, hosted on GitHub Pages.

[Live site →](https://sainiamandeepsingh.github.io/)

</td>
</tr>
</table>

### Technical skills

**Data Engineering**
![Microsoft Fabric](https://img.shields.io/badge/-Microsoft%20Fabric-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![Azure Synapse](https://img.shields.io/badge/-Azure%20Synapse-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![Azure Data Factory](https://img.shields.io/badge/-Azure%20Data%20Factory-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![Azure Data Lake](https://img.shields.io/badge/-Azure%20Data%20Lake-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)

**Business Intelligence**
![Power BI](https://img.shields.io/badge/-Power%20BI-F2C811?style=flat-square&logo=powerbi&logoColor=black)
![DAX](https://img.shields.io/badge/-DAX-005A9E?style=flat-square)
![Power Query](https://img.shields.io/badge/-Power%20Query%20(M)-742774?style=flat-square)

**Programming Languages**
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![R](https://img.shields.io/badge/-R-276DC3?style=flat-square&logo=r&logoColor=white)
![SQL](https://img.shields.io/badge/-SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)

**Machine Learning and Data Science**
![scikit-learn](https://img.shields.io/badge/-scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white)
![pandas](https://img.shields.io/badge/-pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![PyTorch](https://img.shields.io/badge/-PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![NumPy](https://img.shields.io/badge/-NumPy-013243?style=flat-square&logo=numpy&logoColor=white)

**Visualisation and Application Development**
![Streamlit](https://img.shields.io/badge/-Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white)
![D3.js](https://img.shields.io/badge/-D3.js-F9A03C?style=flat-square&logo=d3.js&logoColor=white)
![Marimo](https://img.shields.io/badge/-Marimo-30A8FF?style=flat-square)

### Languages

English (Full Professional) · Greek (Native) · Dutch (Professional Working, B1) · Hindi (Professional Working) · Punjabi (Native)

### Repository activity

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/SainiAmandeepSingh/SainiAmandeepSingh/output/github-contribution-grid-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/SainiAmandeepSingh/SainiAmandeepSingh/output/github-contribution-grid-snake.svg">
    <img alt="Contribution snake animation" src="https://raw.githubusercontent.com/SainiAmandeepSingh/SainiAmandeepSingh/output/github-contribution-grid-snake.svg">
  </picture>
</p>

The animation above is generated and committed directly to this repo by a GitHub Action (`.github/workflows/snake.yml`), so it doesn't depend on any third-party server staying online. I dropped the language/streak stats cards that used to sit here — they were served by a shared community-run service (github-readme-stats.vercel.app) that's been failing intermittently due to rate limits on its end.

### Recent activity

<!--START_SECTION:activity-->
<!--END_SECTION:activity-->

### Publication

**[Predictive Analysis of Zomato Restaurant Data Using Machine Learning](https://rpubs.com/singhamandeepsaini/Minor)**
Co-authored research applying machine learning techniques to Zomato restaurant data for predictive modelling and analysis. Published via RPubs as part of the Big Data and Design minor at HU University of Applied Sciences Utrecht.

### Other professional experience

**Co-Founder and Head of Service** · [VERS 010](https://www.vers010.nl) · October 2022 – Present
Co-founded a brasserie in Rotterdam with family. Lead front-of-house operations and staff training, manage scheduling, salary processing, and guest communication, and contribute to marketing strategy. Apply data insights and operational reporting to support continuous improvement.

**Junior Data Engineer** · LeydenJar Technologies · September 2024 – March 2025
Built ETL pipelines automating data ingestion from XRF, White light, and OCS laboratory devices into Azure Data Lake. Developed data models and Power BI dashboards to support quality control on a 100% silicon anode product.

**Junior Data Scientist** · Basic-Fit · February 2023 – August 2023
Developed an interactive Power BI dashboard for the software development team within an Agile environment, visualising sprint progress, team performance, and task distribution to support sprint planning and decision-making.

---

### Get in touch

<p align="center">
  <a href="https://www.linkedin.com/in/singhamandeepsainikahma/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"></a>
  <a href="mailto:singhamandeepsaini@hotmail.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"></a>
  <a href="https://sainiamandeepsingh.github.io/"><img src="https://img.shields.io/badge/Portfolio-2F81F7?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Portfolio"></a>
</p>

<p align="center">
  Based in Barendrecht, Netherlands · Open to professional connections and academic collaboration.
</p>
